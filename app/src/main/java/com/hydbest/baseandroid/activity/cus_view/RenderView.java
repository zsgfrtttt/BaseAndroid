...
public class RenderView extends SurfaceView implements SurfaceHolder.Callback {
    ...
    private GLTHread mRenderThread;
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        mRenderThread = new GLTHread(holder);
        mRenderThread.start();
    }
    ...
    class GLTHread extends Thread {
        private final SurfaceHolder mHoder;
        private boolean mIsRunnig = true;

        GLTHread(SurfaceHolder mHoder) {
            this.mHoder = mHoder;
        }
            ...
            while (true) {
                synchronized (mLock) {
                    while (true) {
                        ...
                        Canvas canvas = mHoder.lockCanvas();
                        if (canvas != null) {
                            drawRender(canvas,startTime);
                            mHoder.unlockCanvasAndPost(canvas);
                        }
                    }
                }
            }
    ...
    private void drawRender(Canvas canvas,long mStartTime) {
        if (mSampleX == null) {
        ...
        for (int i = 0; i <= SAMPLE_SIZE; i++) {
            float x = mSampleX[i];
            mOffset = (System.currentTimeMillis() - mStartTime)/400F;

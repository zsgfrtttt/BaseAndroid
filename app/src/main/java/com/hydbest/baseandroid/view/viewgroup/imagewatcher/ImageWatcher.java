...
public class ImageWatcher extends FrameLayout implements GestureDetector.OnGestureListener, ViewPager.OnPageChangeListener {
    ...
    private DelProvider delProvider;
    ...
    public ImageWatcher(Context context, AttributeSet attrs) {
        ...
        setDelProvider(new DefaultDelProvider());
    ...
    public void setDelProvider(DelProvider provider) {
        this.delProvider = provider;
        if (delProvider != null) {
            if (delView != null) removeView(delView);
            delView = delProvider.initialView(getContext());
            addView(delView);
        }
    }
    ...
    public interface DelProvider {
        View initialView(Context context);
    }

    public class DefaultDelProvider implements DelProvider {
        ...
        public View initialView(Context context) {
            ...
            LayoutParams lpCurrentIdx = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
            lpCurrentIdx.gravity = Gravity.TOP | Gravity.RIGHT;
            iv.setLayoutParams(lpCurrentIdx);
            DisplayMetrics displayMetrics = context.getResources().getDisplayMetrics();
            float tCurrentIdxTransY = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 30, displayMetrics) + 0.5f;
            iv.setTranslationY(tCurrentIdxTransY);
            ...
            iv.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    Log.i("csz", "del");
                    if (delListener != null) {
                        delListener.onDel(vPager.getCurrentItem());
                    }
                }
            });
            return iv;
    ...
    private void onUp(MotionEvent e) {
        if (mTouchMode == TOUCH_MODE_AUTO_FLING) {
            ...
            if (delView != null && delListener != null) {
                Rect rect = new Rect();
                delView.getHitRect(rect);
                if (Math.abs(mDownX - e.getX()) < mTouchSlop && rect.contains((int) e.getX(), (int) e.getY())) {
                    delListener.onDel(getCurrentPosition());
                }
            }
